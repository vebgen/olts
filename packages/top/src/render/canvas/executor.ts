...
export type ReplayImageOrLabelArgs = [
    ...
    number,
    import("../canvas").Label | HTMLImageElement | HTMLCanvasElement | HTMLVideoElement,
...
/**
 * @param replayImageOrLabelArgs Arguments to replayImageOrLabel
 * @return Declutter bounding box.
 */
function getDeclutterBox(
    replayImageOrLabelArgs: ReplayImageOrLabelArgs
): BBox {
...
export class Executor {
     ...
     */
    constructor(resolution: number, pixelRatio: number, overlaps: boolean, instructions: import("../canvas").SerializableInstructions) {
        /**
         * @protected
         * @type {boolean}
         */
        ...
        /**
         * @private
         * @type {!Record<number,Coordinate|Coordinate[]|Array<Coordinate[]>>}
         */
    ...
    /**
     * @param {string|string[]} text Text.
     * @param textKey Text style key.
     * @param fillKey Fill style key.
     * @param strokeKey Stroke style key.
    ...
    createLabel(text: string | string[], textKey: string, fillKey: string, strokeKey: string): import("../canvas").Label {
        ...
        const label: import("../canvas").Label = {
     ...
     * @param {Coordinate} p1 1st point of the background box.
     * @param {Coordinate} p2 2nd point of the background box.
     * @param {Coordinate} p3 3rd point of the background box.
     * @param {Coordinate} p4 4th point of the background box.
    ...
    replayTextBackground_(
        ...
        if (fillInstruction) {
            this.alignAndScaleFill_ = /** @type */ (fillInstruction[2]);
            this.fill_(context);
        }
    ...
    /**
     * @private
     * @param sheetWidth Width of the sprite sheet.
     * @param sheetHeight Height of the sprite sheet.
     * @param centerX X.
     * @param centerY Y.
     * @param width Width.
     * @param height Height.
     ...
     * @param {CanvasRenderingContext2D} context Context.
     */
    fill_(context: CanvasRenderingContext2D) {
    ...
    setStrokeStyle_(context: CanvasRenderingContext2D, instruction: any[]) {
      ...
      /** @type {import("../../colorlike").ColorLike} */ (instruction[1]);
        context.lineWidth = /** @type */ (instruction[2]);
        context.lineCap = /** @type {CanvasLineCap} */ (instruction[3]);
        context.lineJoin = /** @type {CanvasLineJoin} */ (instruction[4]);
        context.miterLimit = /** @type */ (instruction[5]);
        context.lineDashOffset = /** @type */ (instruction[7]);
        context.setLineDash(/** @type {number[]} */(instruction[6]));
    }

    /**
     * @private
     * @param {string|string[]} text The text to draw.
     * @param textKey The key of the text state.
     * @param strokeKey The key for the stroke state.
     * @param fillKey The key for the fill state.
     ...
     * @param {Extent} [hitExtent] Only check
    ...
    execute_<T>(
        ...
        const state = /** @type {import("../../render").State} */ ({
            context: context,
        ...
        let /** @type {FeatureLike} */ feature: FeatureLike;
        ...
        while (i < ii) {
            ...
            switch (type) {
                case CanvasInstruction.BEGIN_GEOMETRY:
                    feature = /** @type {FeatureLike} */ (
                        instruction[1]
                    );
                    currentGeometry = instruction[3];
                    if (!feature.getGeometry()) {
                        i = /** @type */ (instruction[2]);
                    } else if (
                        hitExtent !== undefined &&
                        !intersects(hitExtent, currentGeometry.getExtent())
                    ) {
                        i = /** @type */ (instruction[2]) + 1;
                    } else {
                        ++i;
                    }
                    break;
            ...
            /** @type {HTMLCanvasElement|HTMLVideoElement|HTMLImageElement} */ (
                    ...
                    anchorX = /** @type */ (instruction[4]);
                    anchorY = /** @type */ (instruction[5]);
                    let height = /** @type */ (instruction[6]);
                    const opacity = /** @type */ (instruction[7]);
                    const originX = /** @type */ (instruction[8]);
                    const originY = /** @type */ (instruction[9]);
                    const rotateWithView = /** @type {boolean} */ (instruction[10]);
                    let rotation = /** @type */ (instruction[11]);
                    const scale = /** @type {Size} */ (
                        instruction[12]
                    );
                    let width = /** @type */ (instruction[13]);
                    const declutterMode =
            ...
            /** @type {import("../canvas").DeclutterImageWithText} */ (
                    ...
                    if (!image && instruction.length >= 20) {
                        // create label images
                        text = /** @type */ (instruction[19]);
                        textKey = /** @type */ (instruction[20]);
                        strokeKey = /** @type */ (instruction[21]);
                        fillKey = /** @type */ (instruction[22]);
                        ...
                        instruction[3] = image;
                        const textOffsetX = /** @type */ (instruction[23]);
                        anchorX = (labelWithAnchor.anchorX - textOffsetX) * this.pixelRatio;
                        instruction[4] = anchorX;
                        const textOffsetY = /** @type */ (instruction[24]);
                    ...
                    if (instruction.length > 25) {
                        geometryWidths = /** @type */ (instruction[25]);
                    }
                    ...
                    for (; d < dd; d += 2) {
                        ...
                        const args: ReplayImageOrLabelArgs = [
                            context,
                            contextScale,
                            image,
                            dimensions,
                ...
                case CanvasInstruction.DRAW_CHARS:
                    const begin = /** @type */ (instruction[1]);
                    const end = /** @type */ (instruction[2]);
                    const baseline = /** @type */ (instruction[3]);
                    const overflow = /** @type */ (instruction[4]);
                    fillKey = /** @type */ (instruction[5]);
                    const maxAngle = /** @type */ (instruction[6]);
                    const measurePixelRatio = /** @type */ (instruction[7]);
                    const offsetY = /** @type */ (instruction[8]);
                    strokeKey = /** @type */ (instruction[9]);
                    const strokeWidth = /** @type */ (instruction[10]);
                    text = /** @type */ (instruction[11]);
                    textKey = /** @type */ (instruction[12]);
