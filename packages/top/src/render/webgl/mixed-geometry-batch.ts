...
/**
 * Object that holds a reference to a feature as well as the raw coordinates
 * of its various geometries
 */
...
export class MixedGeometryBatch {
   ...
   * @param {import("../../proj").TransformFunction} [projectionTransform] Projection transform.
   */
  addFeatures(features: Array<Feature | RenderFeature>, projectionTransform: import("../../proj").TransformFunction) {
   ...
   * @param {import("../../proj").TransformFunction} [projectionTransform] Projection transform.
   */
  addFeature(feature: Feature | RenderFeature, projectionTransform: import("../../proj").TransformFunction) {
   ...
   * @private
   */
  clearFeatureEntryInPointBatch_(feature: Feature | RenderFeature): GeometryBatchItem | void {
   ...
   * @private
   */
  clearFeatureEntryInLineStringBatch_(feature: Feature | RenderFeature): GeometryBatchItem | void {
   ...
   * @private
   */
  clearFeatureEntryInPolygonBatch_(feature: Feature | RenderFeature): GeometryBatchItem | void {
   ...
   * @private
   */
  addGeometry_(geometry: import("../../geom").Geometry | RenderFeature, feature: Feature | RenderFeature) {
    ...
    switch (type) {
      ...
      case 'Polygon': {
        const polygonGeom =
          /** @type {import("../../geom").Polygon|RenderFeature} */ (
            geometry
          );
        this.addCoordinates_(
          type,
      ...
      case 'Point': {
        const pointGeom = /** @type {import("../../geom").Point} */ (
          geometry
        );
        this.addCoordinates_(
          type,
   ...
   * @private
   */
  addRefToEntry_(featureUid: string, entry: GeometryBatchItem): GeometryBatchItem {
   ...
   * @param {Feature|RenderFeature} feature Feature
   */
  changeFeature(feature: Feature | RenderFeature) {
   ...
   * @param {Feature|RenderFeature} feature Feature
   */
  removeFeature(feature: Feature | RenderFeature) {
    let entry;
